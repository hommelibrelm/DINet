...
from mmdet.core import (RegionAssigner, build_assigner, build_sampler,
                        images_to_levels, multi_apply, unmap, anchor_inside_flags)
...
class StageCascadeRPNHeadOln(RPNHead):
    ...
    def __init__(self,
        ...
        super(StageCascadeRPNHeadOln, self).__init__(
            in_channels,
    ...
    def forward_single(self, x, offset):
        ...
        objectness_pred = self.rpn_obj(x)
        return bridged_x, cls_score, bbox_pred, objectness_pred
    ...
    def region_targets(self,
        ...
        """See :func:`StageCascadeRPNHeadOln.get_targets`."""
    ...
    def loss_single(self, cls_score, bbox_pred, objectness_score, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, objectness_targets, objectness_weights, num_total_samples):
        ...
        loss_objectness = self.loss_objectness(
            ...
            avg_factor=num_total_samples)
        if self.with_cls:
            return loss_cls, loss_reg, loss_objectness
        return None, loss_reg, loss_objectness
    ...
    def loss(self,
        ...
        cls_reg_targets = self.get_targets_all(
            anchor_list,
            valid_flag_list,
            gt_bboxes,
            img_metas,
        ...
        if self.with_cls:
            return dict(loss_rpn_cls=losses[0], loss_rpn_reg=losses[1], loss_rpn_objectness=losses[2])
        return dict(loss_rpn_reg=losses[1], loss_rpn_objectness=losses[2])
        ...
        for img_id in range(len(img_metas)):
            ...
            if with_nms:
                ...
                proposals = self._get_bboxes_single(cls_score_list,
                                                    bbox_pred_list,
                                                    objectness_score_list,
                                                    anchor_list[img_shape], img_shape,
                                                    scale_factor, cfg, rescale)
            else:
                proposals = self._get_bboxes_single(cls_score_list,
                                                    bbox_pred_list,
...
@HEADS.register_module()
class CascadeRPNHeadOln(BaseDenseHead):
    """The CascadeRPNHeadOln will predict more accurate region proposals, which is
    ...
    def __init__(self, num_stages, stages, train_cfg, test_cfg, init_cfg=None):
        super(CascadeRPNHeadOln, self).__init__(init_cfg)
        ...
        for i in range(len(stages)):
            print("train_cfg[i] is ",train_cfg[i])
    ...
    def loss(self):
        """loss() is implemented in StageCascadeRPNHeadOln."""
        pass

    def get_bboxes(self):
        """get_bboxes() is implemented in StageCascadeRPNHeadOln."""
        pass
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if stage.adapt_cfg['type'] == 'offset':
                ...
                offset_list = None
            x, cls_score, bbox_pred, objectness_pred= stage(x, offset_list)
            rpn_loss_inputs = (anchor_list, valid_flag_list, cls_score,
                               bbox_pred, objectness_pred, gt_bboxes, img_metas)
        ...
        if proposal_cfg is None:
            ...
            proposal_list = self.stages[-1].get_bboxes(anchor_list, cls_score,
                                                       bbox_pred, objectness_pred, img_metas,
                                                       self.test_cfg)
    ...
    def aug_test_rpn(self, x, img_metas):
        ...
        raise NotImplementedError(
            'CascadeRPNHeadOln does not support test-time augmentation')
