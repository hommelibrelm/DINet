...
rpn_head=dict(
        _delete_=True,
        type='CascadeRPNHeadDecoupled',
        num_stages=2,
        stages=[
            dict(
                type='StageCascadeRPNHeadDecoupled',
                in_channels=256,
                feat_channels=256,
...
#     _delete_=True, grad_clip=dict(max_norm=900, norm_type=2))
...
optimizer = dict(type='SGD', lr=0.18, momentum=0.9, weight_decay=0.0001)
lr_config = dict(
    policy='step',
    warmup='linear',
    warmup_iters=500,
...
work_dir='./work_dirs/oln_box_decoupled/'
